...
export function ProtectedShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-10 hidden flex-col border-r border-white/10 bg-black/60 backdrop-blur-2xl sm:flex transition-[width] duration-200",
      ...
      >
        <div className={cn("flex items-center gap-3 px-5 py-5", collapsed && "justify-center px-3")}> 
          ...
          <div className={cn("flex flex-col", collapsed && "hidden")}> 
            <span className="text-sm font-heading font-semibold text-foreground">BuyBox Guard</span>
          ...
          <Button
            ...
            className={cn("ml-auto h-8 w-8 hover:bg-white/5", collapsed && "ml-0")}
          >
            {collapsed ? <ChevronsRight className="h-4 w-4" /> : <ChevronsLeft className="h-4 w-4" />}
          </Button>
        </div>
        <div className={cn("mx-4 h-px bg-white/10", collapsed && "mx-3")} />
      ...
      <div className={cn("flex min-w-0 flex-1 flex-col", collapsed ? "sm:pl-[72px]" : "sm:pl-[220px]")}>
        {/* Mobile Header */}
        <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b border-white/10 bg-black/70 px-4 backdrop-blur-2xl sm:hidden">
          <Sheet>
